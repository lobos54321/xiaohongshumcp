...
// ProcessImagesWithContext 处理图片列表，支持上下文取消
func (p *ImageProcessor) ProcessImagesWithContext(ctx context.Context, images []string) ([]string, error) {
    var localPaths []string
    var urlsToDownload []string
    ...
    if len(urlsToDownload) > 0 {
        downloadedPaths, err := p.downloader.DownloadImagesWithContext(ctx, urlsToDownload)
        if err != nil {
            return nil, fmt.Errorf("failed to download images: %w", err)
        }
        localPaths = append(localPaths, downloadedPaths...)
