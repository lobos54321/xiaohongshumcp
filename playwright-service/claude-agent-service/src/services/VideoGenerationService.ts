...
export class VideoGenerationService {
     ...
     * 导出编辑后的视频（烧字幕 + 混音 + 调速）
     */
    async exportWithEdits(params: {
        videoUrl: string;
        subtitles?: Array<{ id: string; startTime: number; endTime: number; text: string }>;
        backgroundMusic?: { name: string; url: string; volume: number } | null;
        playbackSpeed?: number;
    }): Promise<{ success: boolean; exportedUrl?: string; error?: string }> {
        const { videoUrl, subtitles, backgroundMusic, playbackSpeed = 1.0 } = params;
        const id = Date.now();
        const videoPath = join(tmpdir(), `export_src_${id}.mp4`);
        const srtPath = join(tmpdir(), `export_subs_${id}.srt`);
        const bgmPath = join(tmpdir(), `export_bgm_${id}.mp3`);
        const outputPath = join(tmpdir(), `export_out_${id}.mp4`);
        const tempFiles = [videoPath, srtPath, bgmPath, outputPath];

        try {
            console.log('[VideoExport] Starting export with edits...');
            ...
            if (backgroundMusic?.url) {
                try {
                    const bgmRes = await fetch(backgroundMusic.url);
                    if (bgmRes.ok) {
                    ...
                    console.warn('[VideoExport] Failed to download BGM, skipping:', e);
                }
            }
            ...
            await new Promise<void>((resolve, reject) => {
                ...
                if (hasSrt) {
                    ...
                    filters.push(`subtitles='${escapedSrtPath}'`);
                }
                ...
                if (playbackSpeed !== 1.0) {
                    ...
                    // atempo 范围 0.5-2.0，需要链式处理
                    while (speed > 2.0) { atempoFilters.push('atempo=2.0'); speed /= 2.0; }
                    while (speed < 0.5) { atempoFilters.push('atempo=0.5'); speed *= 2.0; }
                    atempoFilters.push(`atempo=${speed.toFixed(4)}`);
                    cmd = cmd.audioFilters(atempoFilters);
                }

                if (hasBgm) {
                    const vol = backgroundMusic?.volume ?? 0.3;
                ...
                cmd.outputOptions(['-c:v', 'libx264', '-preset', 'fast', '-crf', '23', '-c:a', 'aac', '-b:a', '128k'])
                    .output(outputPath)
                    .on('end', () => resolve())
                    .on('error', (err: Error) => reject(new Error(`ffmpeg export error: ${err.message}`)))
                    .run();
            });

            // 5. 上传到 Supabase Storage
            const outputBuffer = await readFile(outputPath);
            const fileName = `exported_${id}.mp4`;
            ...
            console.log(`[VideoExport] ✅ Export complete: ${publicUrl.publicUrl}`);
